...
type ecsClient interface {
	...
	DescribeInstanceStatus(
		params *ecs.DescribeInstanceStatusRequest) (*ecs.DescribeInstanceStatusResponse, error)
	// Describe Images
	DescribeImages(
		params *ecs.DescribeImagesRequest) (*ecs.DescribeImagesResponse, error)
	// Create NIC
	...
	DescribeNetworkInterfaces(
		params *ecs.DescribeNetworkInterfacesRequest) (*ecs.DescribeNetworkInterfacesResponse, error)
	// Atttach NIC
...
type vpcClient interface {
	CreateNatGateway(
		params *vpc.CreateNatGatewayRequest) (*vpc.CreateNatGatewayResponse, error)
	DeleteNatGateway(
		params *vpc.DeleteNatGatewayRequest) (*vpc.DeleteNatGatewayResponse, error)
	CreateSnatEntry(
		params *vpc.CreateSnatEntryRequest) (*vpc.CreateSnatEntryResponse, error)
	...
	GetNatGatewayAttribute(
		params *vpc.GetNatGatewayAttributeRequest) (*vpc.GetNatGatewayAttributeResponse, error)
	AllocateEipAddress(
	...
	DescribeEipGatewayInfo(
		params *vpc.DescribeEipGatewayInfoRequest) (*vpc.DescribeEipGatewayInfoResponse, error)
...
func waitInstance(instanceID string, ecsClient ecsClient) error {
	req := &ecs.DescribeInstanceAttributeRequest{
		InstanceId: tea.String(instanceID),
	}

	for range 20 {
		_, err := ecsClient.DescribeInstanceAttribute(req)
		if err == nil {
			return nil
		}

		if err.(*tea.SDKError).Code == tea.String("InvalidInstanceId.NotFound") {
...
func getIPs(instanceId string, ecsClient ecsClient) ([]netip.Addr, error) {
	var podNodeIPs []netip.Addr

	// describe instnace to get the private IP address
	resp, err := ecsClient.DescribeInstanceAttribute(&ecs.DescribeInstanceAttributeRequest{
	...
	for i, addr := range resp.Body.VpcAttributes.PrivateIpAddress.IpAddress {
...
func (p *alibabaCloudProvider) CreateInstance(ctx context.Context, podName, sandboxID string, cloudConfig cloudinit.CloudConfigGenerator, spec provider.InstanceTypeSpec) (*provider.Instance, error) {
	...
	if err := waitInstance(instanceID, p.ecsClient); err != nil {
		logger.Printf("failed to wait for instance %s to be ready: %v", instanceID, err)
		return nil, err
	}

	ips, err := getIPs(*result.Body.InstanceIdSets.InstanceIdSet[0], p.ecsClient)
	if err != nil {
...
func (p *alibabaCloudProvider) DeleteInstance(ctx context.Context, instanceID string) error {
	go func() {
		...
		for range 120 {
			...
			if err != nil {
				if err.(*tea.SDKError).Code == tea.String("IncorrectInstanceStatus") {
...
func (p *alibabaCloudProvider) getPublicIP(_ context.Context, instanceID string) (netip.Addr, error) {
	...
	for range 20 {
		...
		if err != nil {
			return netip.Addr{}, fmt.Errorf("failed to describe instance %q: %w", instanceID, err)
		}
	...
	return netip.ParseAddr(*resp.Body.PublicIpAddress.IpAddress[0])
}
